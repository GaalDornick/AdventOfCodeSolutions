...
let dirOffsets = new Map([
    ['>', [1, 0]],
    ['<', [-1, 0]],
    ['^', [0, 1]],
    ['v', [0, -1]],
]);
const moveToNext = (houses, dir) => {
    const last = houses[houses.length - 1];
    const offset = dirOffsets.get(dir);
    const next = [last[0] + offset[0], last[1] + offset[1]];
    houses.push(next);
    return houses;
};
const uniq = (arr) => {
    return arr.reduce((u, h) => {
        if (u.findIndex(e => e[0] == h[0] && e[1] == h[1]) == -1)
            u.push(h);
        return u;
    }, []);
};
const split = (arr) => {
    ...
    return arr.reduce((result, elem, index) => {
        result[index % 2].push(elem);
        return result;
    }, start);
};
puzzles.forEach(directions => {
    const start = [[0, 0]];
    const housesVisited = [...directions].reduce(moveToNext, start);
    //console.log(housesVisited)
    const uniqueHouses = uniq(housesVisited);
    console.log(uniqueHouses);
    console.log(uniqueHouses.length);
    const splitDirections = split([...directions]);
    const splitHousesVisited1 = splitDirections[0].reduce(moveToNext, [[0, 0]]);
    const splitHousesVisited2 = splitDirections[1].reduce(moveToNext, [[0, 0]]);
    const splitHousesVisited = splitHousesVisited1.concat(splitHousesVisited2);
    const uniqueHouses2 = uniq(splitHousesVisited);
    console.log(uniqueHouses2);
    console.log(uniqueHouses2.length);
});
