let puzzles: Array<Array<[number,number,number]>> = [
    [
        [2,3,4],
        [1,1,10]
    ],
...
function paperRequired(l: number,w:number,h:number) : number {
    return 2*l*w + 2*l*h + 2*w*h + Math.min(l*w , l*h, w*h)
}
function ribbonRequired(l: number,w:number,h:number) : number {
