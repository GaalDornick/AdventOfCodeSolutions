...
object Day17Part2 {
  ...
  def valueForPoint(point: (Int, Int, Int, Int), grid: Map[(Int, Int, Int, Int), Int]) = {
    grid.getOrElse(point, 0)
  }
  def neighbors(point:(Int, Int, Int, Int)) : Seq[(Int, Int, Int, Int)] = {
    val (w, z, y, x) = point
    (w-1 to w+1).flatMap(nw =>{
