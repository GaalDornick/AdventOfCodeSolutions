...
import Data.List (sort, elem, find)
import Debug.Trace (trace)
import Data.Maybe (fromMaybe, isNothing)

digitsBySegmentCount :: Map.Map Int [Int]
-- 1 -> 2, 2 -> 5, 3 -> 5, 4 -> 4, 5-> 5, 6 -> 6, 7 -> 3, 8 -> 7, 9 -> 6, 0 -> 6 
digitsBySegmentCount = Map.fromList([(2, [1]), (3, [7]), (4, [4]), (5, [2,3,5]), (6, [6, 9, 0]), (7, [8])])
...
assignDigits :: String -> (String, [Int])
assignDigits s = let numSegments = length(s)
                     possibleDigits = Map.findWithDefault [] numSegments digitsBySegmentCount
                 in (s, possibleDigits)
...
sanitizeMulti :: [String] -> [String]
sanitizeMulti s = map(sanitize) s

sanitizePattern :: ([String], [String]) -> ([String], [String])
sanitizePattern (training, reading) = ( sanitizeMulti(training), sanitizeMulti(reading) )

{-
processPattern :: ([String], [String]) -> [(String, [Int])]
processPattern (trainings, readings) = let digitsFromTraining = Map.fromList( map(assignDigits) trainings )
                                        in map(\reading -> (reading, Map.findWithDefault [] reading digitsFromTraining)) readings

findNumDigitOccurences :: Int -> (String, [Int]) -> Int
findNumDigitOccurences n (_, digits) = if(n `elem` digits) then 1 else 0


findNumDigitOccurencesInReading :: Int -> [(String, [Int])] -> [Int]
findNumDigitOccurencesInReading n readings = map(findNumDigitOccurences(n)) readings
-}
...
findElemByCount :: Ord k => (Integer -> Bool) -> Histogram k -> [k]
findElemByCount matchFunc hist = Map.keys( Map.filter (matchFunc) hist )
...
type SegmentHistogram = Histogram Char

segmentHistogram :: [String] -> SegmentHistogram
segmentHistogram s = let mergeSegments = sort( foldl (++) "" s)
                        in histogram mergeSegments

diff :: String -> String -> String
diff s1 s2 = filter (\c -> isNothing( find (==c) s2 )) s1



type Cipher = Map.Map Char Char

trainingToCipher :: [String] -> Cipher
{-
trainingToCipher trainings = Map.fromList([
                                                ('d','a'),
                                                ('e','f'),
                                                ('b','c'),
                                                ('g','b'),
...
traceCall v = trace(show(v)) v

trainingToCipher trainings = let trainingHistogram = segmentHistogram trainings
                                 seven = head( filter (\t -> length(t)==3) trainings)
                                 one = head( filter (\t -> length(t)==2) trainings)
                                 four = head( filter (\t -> length(t)==4) trainings)
                                 eight = head( filter (\t -> length(t)==7) trainings)
                                 a = head( diff seven one)
                                 f = head( findElemByCount (==9) trainingHistogram)
                                 c = head( diff(findElemByCount (==8) trainingHistogram)([a]))
                                 b = head( findElemByCount (==6) trainingHistogram)
                                 e = head( findElemByCount (==4) trainingHistogram)
                                 d = head( diff four (b:c:f:[]))
                                 g = head( diff eight (a:b:c:d:e:f:[]))
                             in Map.fromList([(a, 'a'),(b, 'b'),(c, 'c'),(d, 'd'),(e, 'e'),(f, 'f'),(g ,'g')])

readDigit :: String -> Char
readDigit d = case d of "abcefg" -> '0'
                        "cf" -> '1'
                        "acdeg" -> '2'
                        "acdfg" -> '3'
                        "bcdf" -> '4'
...
readDigits digits = map readDigit digits

decryptReading :: Cipher -> String -> String
decryptReading cipher reading = sort(map(\c -> Map.findWithDefault ' ' c cipher) reading)
    
decryptReadings :: Cipher -> [String] -> [String]
decryptReadings cipher readings = map(decryptReading(cipher)) readings

computeReadingValue :: ([String], [String]) -> String
computeReadingValue (trainings, readings) = let cipher = trainingToCipher trainings
                                                decryptedDigits = decryptReadings cipher readings 
                                            in  readDigits decryptedDigits

countOccurences :: (Char -> Bool) -> String -> Int
countOccurences matchFunc s = length( filter matchFunc s)

main :: IO ()
main = do
    putStrLn(show(input))
    let sanitizedInput = map(sanitizePattern) input
    putStrLn(show(sanitizedInput))

    let readingValues = map computeReadingValue  sanitizedInput
    putStrLn(show(readingValues))

    let easyCharCounts = map (countOccurences(\c -> c == '1' || c=='4' || c == '7' || c == '8')) readingValues
    putStrLn(show(easyCharCounts))
    putStrLn(show(sum(easyCharCounts)))

    let intReadingValues= map(read)readingValues
    putStrLn(show(intReadingValues))
    putStrLn(show(sum(intReadingValues)))
