...
const convert = (instr: string): [string,number][] => {
    if(instr=='noop') {
        return [[instr,0]]
    } else {
...
const flatten = <T>(acc: T[], next: T[]) : T[] => {
    acc.push(...next)
    return acc
}
const computeX = (acc: [string,number][], next: [string,number]) : [string,number][] => {
    const curr = (acc[acc.length-1]?? ['',1] as [string, number])[1]
    acc.push([next[0], next[1]+curr])
    return acc
}
puzzles.forEach(instructions => {
    const Xs =  instructions.map(convert).reduce(flatten, [] as [string,number][]).reduce(computeX, [] as [string,number][])
