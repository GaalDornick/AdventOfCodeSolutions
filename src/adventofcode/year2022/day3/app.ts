...
function intersection(...s: string[]) : string {
...
function lag<T>(arr: Array<T>, size: number, step: number = 1) : Array<Array<T>> {
...
let compartmentizedBags: Array<[string, string]> = bags.map(bag => [bag.slice(0, bag.length/2), bag.slice(bag.length/2)])
let commonItems: Array<[[string, string], string]> = compartmentizedBags.map(b => [b, intersection(b[0], b[1]).charAt(0)])
let scores: Array<[[string, string], string, number]> = commonItems.map(i => [i[0], i[1], items.indexOf(i[1]) + 1])
